...
const PostDetail = () => {
    ...
    // 해당 게시글 내용 불러오기 
    const getPost = () => {
        const url = `http://192.168.0.36:8080/api/find/${pstgSeq}`;
        const body = {
            pstgTitle: pstgTitle,
            pstgCn: pstgCn,
    ...
    // 게시글 삭제
    const handleDelete = () => {
        const postUrl = `http://192.168.0.36:8080/api/delete/${pstgSeq}`;
    ...
    // 게시글 수정
    const handleUpdate = () => {
        const url = `http://192.168.0.36:8080/api/update/${pstgSeq}`;
